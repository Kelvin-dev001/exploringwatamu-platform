      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mb-12">
        {cards.map(card => (
          <Link to={card.to} key={card.label}>
            <div className={`card bg-${card.color} shadow-xl text-white hover:scale-105 transition-transform`}>
              <div className="card-body items-center justify-center">
                <h2 className="card-title text-lg">{card.label}</h2>
                <div className="text-4xl font-bold">
                  {loadingStats ? <span className="loading loading-spinner" /> : card.value}
                </div>
              </div>
            </div>
          </Link>
              ...
              <tbody>
                {recentBookings.map(b => (
                  <tr key={b._id || b.id}>
                    ...
                    <td>
                      <span className={`badge badge-sm ${b.status === "booked" ? "badge-success" : b.status === "cancelled" ? "badge-error" : "badge-info"}`}>
                        {b.status || "-"}
                      </span>
                    </td>
                    <td>
                      <Button color="info" onClick={() => window.location.href = `/${b.bookingType?.toLowerCase()}bookings/${b._id}`} size="xs">
                        View
                      </Button>
                    </td>
                  </tr>
