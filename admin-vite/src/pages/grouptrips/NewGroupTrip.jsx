...
export default function NewGroupTrip() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(API_URL + "/group-trips", data, {
        headers: { Authorization: `Bearer ${token}` },
      });
      window.alert("Group trip created!");
      navigate("/grouptrips");
    } catch (err) {
      console.error("Create error:", err?.response?.data || err);
      window.alert(err?.response?.data?.error || "Failed to create group trip.");
    }
    setLoading(false);
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl mx-auto bg-base-100 p-8 shadow rounded">
      <h2 className="text-2xl mb-6 font-bold">Add Group Trip</h2>
      ...
      <div className="mb-4">
        ...
        />
        {heroPreview && (
          <img src={heroPreview} alt="hero preview" className="w-32 h-24 object-cover rounded mt-1" />
        )}
      </div>
      <div className="mb-4">
        ...
        <div className="flex gap-2 flex-wrap">
          {galleryPreview.map((url, i) => (
            <img key={i} src={url} alt={`gallery${i}`} className="w-16 h-12 object-cover rounded" />
          ))}
        </div>
      </div>
