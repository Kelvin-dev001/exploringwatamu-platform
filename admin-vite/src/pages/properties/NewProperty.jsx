...
export default function NewProperty() {
  const [form, setForm] = useState({
    type: "villa", name: "", description: "", location: "", price: "", active: true
  });
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      console.error("Property create error:", err?.response?.data || err);
      window.alert(err?.response?.data?.error || "Failed to create property.");
    }
    setLoading(false);
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-xl mx-auto bg-base-100 p-8 shadow rounded">
      ...
      <div className="mb-3">
        ...
        <div className="flex gap-2">{picturePreview.map((url, i) => <img key={i} src={url} alt={`preview${i}`} className="w-16 h-10 object-cover rounded" />)}</div>
      </div>
      <div className="mb-3">
        ...
        <div className="flex gap-2">{documentPreview.map((url, i) => <span key={i} className="inline-block w-16 h-10 bg-base-200 rounded text-center leading-10">Doc {i + 1}</span>)}</div>
      </div>
